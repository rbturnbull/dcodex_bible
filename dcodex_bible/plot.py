...
def plot_affiliation_matrix(
    ...
    if (
        not force_compute
        and matrix_filename
        and isfile(matrix_filename)
        and access(matrix_filename, R_OK)
    ):
    ...
    chapter_beginnings = BibleVerse.objects.filter(
        id__gt=verse_min.id, id__lt=verse_max.id, verse=1
    )
    ...
    for manuscript_index, manuscript in enumerate(manuscripts):
        ...
        ax.fill_between(
            x_values,
            manuscript_index + alpha * matrix[manuscript_index],
            manuscript_index - alpha * matrix[manuscript_index],
            color=color,
        )
